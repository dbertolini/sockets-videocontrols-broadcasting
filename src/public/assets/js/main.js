...
window.onload = () => { 
    ...
    document.getElementsByClassName("vjs-progress-control vjs-control")[0].onclick = () => {
        socket.emit('setTime', {
            time: myVideo.currentTime
        });
    };
    document.getElementsByClassName("vjs-progress-holder vjs-slider vjs-slider-horizontal")[0].onclick = () => {
        socket.emit('setTime', {
            time: myVideo.currentTime
        });
    };
